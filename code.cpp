...
void subsetsUtil(int n, vector<vector<int>> &res, vector<int> &subset, int index)
{
...
vector<vector<int>> subsets(int n)
{
    ...
    int index = 0;
    subsetsUtil(n, res, subset, index);
...
int main()
{
    ...
    */
    ...
    vector<vector<int>> totalSubset = subsets(cust);
    for (int i = 0; i < totalSubset.size(); i++)
    {
        removeEle(dislike, totalSubset[i], total, cust);
    }
    cout << maxCust << endl;
